"""
Stage 15: Z-gate — Mamba y×SiLU(z) 유사체 효과 검증

Mamba 핵심 메커니즘 비교:
  - conv1d : 로컬 n-gram 패턴 (Stage 14에서 검증)
  - Z-gate : 입력이 상태를 열고/닫음 → x = x × SiLU(W_z u)

Stage 14 best variant (slim+conv-K4) 기준으로:
  A) slim+conv-K4          : Stage 14 best (기준)
  B) slim+gate-K4          : Z-gate only (gate가 단독으로 얼마나 도움?)
  C) slim+conv+gate-K4     : conv + gate (핵심 조합)
  D) slim+conv+gate+all-K4 : conv + gate + sel + mom + prior_bias
  E) Mamba                 : 참조
...
def main():
    ...
    # A) conv only (Stage 14 best 기준)
    results["slim+conv-K4"] = train_one(
        make_prism(vocab_size, use_conv=True),
        train_loader, val_loader, args, "slim+conv-K4")

    # B) gate only
    results["slim+gate-K4"] = train_one(
        make_prism(vocab_size, use_gate=True),
        train_loader, val_loader, args, "slim+gate-K4")

    # C) conv + gate (핵심 조합)
    results["slim+conv+gate-K4"] = train_one(
        make_prism(vocab_size, use_conv=True, use_gate=True),
        train_loader, val_loader, args, "slim+conv+gate-K4")

    # D) 전체 누적: conv + gate + sel + mom + prior_bias
    results["slim+conv+gate+all-K4"] = train_one(
        make_prism(vocab_size, use_conv=True, use_gate=True,
                   input_dep_pi=True, prior_bias=True, momentum=0.9),
        train_loader, val_loader, args, "slim+conv+gate+all-K4")
    ...
    print("\n" + "=" * 64)
    print("Stage 15: Z-gate 효과 (낮을수록 좋음)")
    print("=" * 64)
    for name, (ppl, n) in results.items():
        print(f"  {name:<30s}: val_ppl {ppl:.3f}  ({n:,} params)")

    base = results.get("slim+conv-K4", (None,))[0]
    if base:
        for name in ["slim+gate-K4", "slim+conv+gate-K4", "slim+conv+gate+all-K4"]:
            if name in results:
        ...
        if "Mamba" in results:
            ...
            if "slim+conv+gate+all-K4" in results:
                all_ppl = results["slim+conv+gate+all-K4"][0]
                closed = base - all_ppl
